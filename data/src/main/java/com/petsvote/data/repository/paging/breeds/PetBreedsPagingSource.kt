...
import com.petsvote.data.repository.paging.breeds.PetBreedsPagingRepository.Companion.NETWORK_PAGE_SIZE
...
class PetBreedsPagingSource @Inject constructor(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Item> {
        ...
        return try {
            ...
            var results = scope.async { breedsRepository.getPetBreeds(offset, text ?: "", limit, type = type) }.await()

            var nextKey = if (results.size < NETWORK_PAGE_SIZE) {
                null
            } else {
